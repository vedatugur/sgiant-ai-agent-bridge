...
export interface MountAiAgentOptions {
   ...
   *  (same-origin embedding). Pass explicit origins (or `"*"`) to allow an
   *  external embedder — do this deliberately; it grants page control. */
  allowedOrigins?: string[] | "*";
  /** Debounce (ms) for re-publishing the catalog after DOM changes. */
  rescanDebounceMs?: number;
}

export interface AiAgentHandle {
  /** Re-scan and re-publish the catalog now. */
  publish(): void;
  destroy(): void;
}
 ...
 */
export function mountAiAgent(opts: MountAiAgentOptions = {}): AiAgentHandle {
  const inert: AiAgentHandle = { publish: () => {}, destroy: () => {} };
  ...
  if (Array.isArray(allowed) && allowed.length === 1) targetOrigin = allowed[0];

  const post = (msg: AgentMessage): void => {
    try {
  ...
  if (
    ...
    announce();
  else window.addEventListener("DOMContentLoaded", announce, { once: true });
